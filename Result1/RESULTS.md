...
|-------|---------------|
| H2 noise | Increase to `max_tasks=5` per cell for stable verification-mode comparison |
| H3 uncomputable | Lower `is_success` threshold from 0.40 → 0.15, or switch education rubrics to `contains_match` |
| H4 overhead | Profile A1 only for x86 institutional run; it's the only config meeting the 3× goal |
| A4 cost | Consider `max_rounds=1` even in standard mode for A4 to bring overhead below 4× |

---

*Generated by run_result1.py — Frugal AI Agent Swarms, COL Phase-1 Pilot (2026-06-17)*
